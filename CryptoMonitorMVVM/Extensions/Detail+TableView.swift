...
extension DetailViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(
            withIdentifier: TransactionCell.identifire, for: indexPath) as? TransactionCell else {
            fatalError("Error")
        }
        ...
        if let price = buyings.price {
            let formattedPrice = Formatter.shared.format("\(price)", format: .currencyShort)
            cell.priceLabel.text = "\(formattedPrice)"
        }
        let totalCost = viewModel.calcCostOfOnePurchase()
        cell.totalCostLabel.text = Formatter.shared.format(totalCost[indexPath.row], format: .currencyShort)
        return cell
    }
    ...
    func tableView(_ tableView: UITableView, canEditRowAt indexPath: IndexPath) -> Bool {
        ...
        return indexPath.row == rowsInSection - 1 ? true : false
    }
